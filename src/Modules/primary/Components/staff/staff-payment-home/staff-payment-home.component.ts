...
import { StaffPaymentComponent } from '../staff-payment/staff-payment.component';
import { StaffVM } from 'src/Modules/primary/domainModels/staff/StaffVM';

@Component({
  selector: 'app-staff-payment-home',
  templateUrl: './staff-payment-home.component.html',
  styleUrls: ['./staff-payment-home.component.css']
})
export class StaffPaymentHomeComponent implements OnInit {
  // VARIABLES
  columnList: any;
  ...
  DoDBOperation(operationType: DB_OPERATION, item: any): void {
    ...
    switch (operationType) {
      case DB_OPERATION.CREATE:
        URL = ApiUrl.GiveStaffPayment;
        break;
      case DB_OPERATION.READ:
        URL = ApiUrl.StaffPaymentList;
        break;
      case DB_OPERATION.UPDATE:
        URL = ApiUrl.UpdateProduction + '/' + item.Id;
        break;
      case DB_OPERATION.DELETE:
        URL = ApiUrl.DeleteStaffPayment;
        break;
      default:
        break;
    }
  ...
  // MODAL FUNCTION
  openModalAdd() {
    const ref = this.dialogService.open(StaffPaymentComponent, {
      data: {
  ...
  GoToPage(op: any): void {
    switch (op) {
      case '+':
        ...
        this.getDataListVM.PageNumber = this.CurrentPageNo;
        this.getDataListVM.PageSize = this.CurrentPageSize;
        this.getDataListVM.ClientId = this.selectedStaff.Id;
        this.DoDBOperation(DB_OPERATION.READ, this.getDataListVM);
        break;
      case '-':
