...
import { UpdateItemStatusComponent } from '../update-item-status/update-item-status.component';
...
@Component({
  selector: 'app-item-status-mgmt',
  templateUrl: './item-status-mgmt.component.html',
  styleUrls: ['./item-status-mgmt.component.css']
})
export class ItemStatusMgmtComponent implements OnInit {

  // VARIABLES
  columnList: any;
  wrapperItemList: WrapperItemStatusListVM;
  ...
  constructor(private dialogService: DialogService,
    ...
    this.wrapperItemList = new WrapperItemStatusListVM();
...
// DB OPERATION FUNCTION
  DoDBOperation(operationType: DB_OPERATION, item: any): void {
    ...
    switch (operationType) {
      case DB_OPERATION.CREATE:
        URL = ApiUrl.SetItemStatus;
        break;
      case DB_OPERATION.READ:
        URL = ApiUrl.GetItemStatus;
        break;
      case DB_OPERATION.UPDATE:
        URL = ApiUrl.UpdateItemStatus + '/' + item.Id;
        break;
      case DB_OPERATION.DELETE:
        URL = ApiUrl.DeleteItemStatus;
        break;
      default:
        break;
    }
    console.log(URL);
    this.baseService.set<WrapperItemStatusListVM>(URL, item)
      .subscribe((data) => {
  ...
  // MODAL FUNCTION
  openModalAdd() {
    const ref = this.dialogService.open(AddItemStatusComponent, {
      data: {
  ...
  openModalUpdate(item: any) {
    const ref = this.dialogService.open(UpdateItemStatusComponent, {
      data: {
        modelData: item
      },
      header: 'Give necessary  info',
