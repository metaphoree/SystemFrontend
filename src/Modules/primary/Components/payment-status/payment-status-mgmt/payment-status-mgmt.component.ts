...
import { WrapperPaymentStatusListVM } from 'src/Modules/primary/domainModels/payment-status/WrapperPaymentStatusListVM';
import { AddPaymentStatusComponent } from '../add-payment-status/add-payment-status.component';
import { UpdateItemStatusComponent } from '../../item-status/update-item-status/update-item-status.component';
...
@Component({
  selector: 'app-payment-status-mgmt',
  templateUrl: './payment-status-mgmt.component.html',
  styleUrls: ['./payment-status-mgmt.component.css']
})
export class PaymentStatusMgmtComponent implements OnInit {

  // VARIABLES
  columnList: any;
  wrapperItemList: WrapperPaymentStatusListVM;
  ...
  constructor(private dialogService: DialogService,
    ...
    this.wrapperItemList = new WrapperPaymentStatusListVM();
...
// DB OPERATION FUNCTION
  DoDBOperation(operationType: DB_OPERATION, item: any): void {
    ...
    switch (operationType) {
      case DB_OPERATION.CREATE:
        URL = ApiUrl.SetPaymentStatus;
        break;
      case DB_OPERATION.READ:
        URL = ApiUrl.GetPaymentStatus;
        break;
      case DB_OPERATION.UPDATE:
        URL = ApiUrl.UpdatePaymentStatus + '/' + item.Id;
        break;
      case DB_OPERATION.DELETE:
        URL = ApiUrl.DeletePaymentStatus;
        break;
      default:
        break;
    }
    console.log(URL);
    this.baseService.set<WrapperPaymentStatusListVM>(URL, item)
      .subscribe((data) => {
  ...
  // MODAL FUNCTION
  openModalAdd() {
    const ref = this.dialogService.open(AddPaymentStatusComponent, {
      data: {
  ...
  openModalUpdate(item: any) {
    const ref = this.dialogService.open(UpdatePaymentStatusComponent, {
      data: {
        modelData: item
      },
      header: 'Give necessary  info',
